...
export class QuizProgress<MyContext extends Context & SessionFlavor<SessionData> & ConversationFlavor> {
    ...
    getConversation(){
        ...
        return async (conversation: Conversation<MyContext>, ctx: MyContext) => {
            ...
            if(!ctx.session.hasTermsAgreement){
                ...
                conversation.session.hasTermsAgreement = await termsComversation(conversation,ctx)
            ...
            if(self.quiz.isPassed()){
                await ctx.reply(QUIZ_MSG.REPLAY, 
                    {
                        reply_markup: new Keyboard().add({"text": QUIZ_MSG.REPLAY_YES}).add({"text": QUIZ_MSG.REPLAY_NO}).row().add({text: QUIZ_MSG.SHOW_RESULT}).oneTime(true)
                    }
                )
