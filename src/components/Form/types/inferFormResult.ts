...
type InputTypeToTSType<T extends FORM_INPUT_TYPES> =
  T extends FORM_INPUT_TYPES.STRING ? string :
  T extends FORM_INPUT_TYPES.NUMBER ? number :
  T extends FORM_INPUT_TYPES.FLOAT ? number :
  T extends FORM_INPUT_TYPES.BOOLEAN ? boolean :
  T extends FORM_INPUT_TYPES.SELECT ? unknown :
  T extends FORM_INPUT_TYPES.DATE ? string :
  never
...
export type InferFormResult<T extends readonly FormInputProps[]> = MergeInputResults<T>
